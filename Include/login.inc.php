...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($Username) || empty($Password)) {
    header("Location: ../index.php?error=emptyfields&mailuid=".$Username);
    exit();
  }
  else {
...
$sql = "SELECT * FROM `Employee_Records` WHERE `Username` = '$Username'";
    ...
    if (!$result) {
      ...
      header("Location: ../index.php?error=Error Please Try Again Later");
      exit();
    }
      if ($row_cnt > 0) {
        ...
        header("Location: ../index.php?error=Wrong_Username or Password");
        exit();
      }
    }
